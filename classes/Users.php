...
// Change User Password By Id
  public function changePasswordBysingelUserId($userid, $data)
  {
      ...
      // If old_password is provided, we are handling a self-change and need to verify it.
      ...
      // If old_password is not provided, we assume an admin/faculty is changing another user's password.

      // Update the password (using SHA1 hashing as in your current implementation)
      $new_hashed_pass = SHA1($data['new_password']);
      $sql = "UPDATE tbl_users SET password = :password WHERE id = :id";
      $stmt = $this->db->pdo->prepare($sql);
      $stmt->bindValue(':password', $new_hashed_pass);
      $stmt->bindValue(':id', $userid);
      $result = $stmt->execute();
